...
function createWorksheetDocument(d) {
  ...
  const p = (text,options={}) => new Paragraph({...options, children:[new TextRun({text:String(text || ''),font:'Sarabun',size:26,bold:!!options.bold})]});
  const heading = text => p(text,{bold:true,keepNext:true,spacing:{before:240,after:100}});
  const blocks = [];
  ...
  const teacher = w.blocks.some(b=>b.answer) ? [p('เฉลย / แนวทางตรวจสำหรับครู',{pageBreakBefore:true,bold:true}),p(w.title),...w.blocks.flatMap((b,i)=>[heading(`${i+1}. ${b.title}`),p(b.answer)])] : [];
  return new Document({styles:{default:{document:{run:{font:'Sarabun',size:26}}}},sections:[{properties:{page:{size:{width:11906,height:16838},margin:{top:1134,bottom:1134,left:1134,right:1134}}},children:[p('ใบงาน',{bold:true,alignment:AlignmentType.CENTER}),p(w.title,{bold:true,alignment:AlignmentType.CENTER,spacing:{after:180}}),p(`วิชา ${d.subject || ''}    ระดับชั้น ${d.grade || ''}    เวลา ${d.duration || ''}`),p('ชื่อ–สกุล ................................................ ชั้น ........ เลขที่ ........'),p(`ตัวชี้วัด: ${(d.indicators || []).map(i=>i.code).join(' · ')}`),heading('คำชี้แจง'),...w.directions.map(t=>p(t)),...blocks,...teacher]}]});
}
