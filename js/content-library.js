...
(function () {
  ...
  function savedLessonPlanHtml(item) { return `<button class="cl-saved-card lesson-plan" type="button" onclick="openSavedLessonPlan('${escapeHtml(item.id)}')"><span class="cl-saved-type cl-plan-type"><i class="hgi-stroke hgi-presentation-01"></i> แผนการสอน</span><h4>${escapeHtml(item.title || 'แผนการสอนไม่มีชื่อ')}</h4><p>${escapeHtml(item.classLabel || 'ยังไม่ได้เชื่อมห้องเรียน')}</p><div class="cl-saved-meta"><span>${escapeHtml(item.duration || 'ไม่ระบุเวลา')}</span><span>${Array.isArray(item.indicators) ? item.indicators.length : 0} ตัวชี้วัด</span><span>ร่าง</span></div><span class="cl-saved-open">เปิดแผน <i class="hgi-stroke hgi-arrow-right-01"></i></span></button>`; }
  function savedWorksheetHtml(item) { return `<button class="cl-saved-card worksheet" type="button" onclick="openSavedWorksheet('${escapeHtml(item.id)}')"><span class="cl-saved-type cl-worksheet-type"><i class="hgi-stroke hgi-note-02"></i> ใบงาน</span><h4>${escapeHtml(item.title || 'ใบงานไม่มีชื่อ')}</h4><p>${escapeHtml(item.classLabel || 'ยังไม่ได้เชื่อมห้องเรียน')}</p><div class="cl-saved-meta"><span>${escapeHtml(item.worksheetTypeLabel || item.activityType || 'กิจกรรม')}</span><span>${Array.isArray(item.indicators) ? item.indicators.length : 0} ตัวชี้วัด</span><span>ร่าง</span></div><span class="cl-saved-open">เปิดใบงาน <i class="hgi-stroke hgi-arrow-right-01"></i></span></button>`; }
  ...
  window.openWorksheetCreator = function () { state.worksheetView = 'form'; state.worksheetDraft = null; state.worksheetDraftOrigin = 'new'; state.worksheetEditing = false; state.worksheetPrefill = null; state.worksheetType = 'questions'; state.worksheetIndicatorCodes = []; render(); };
  ...
  const worksheetFormats = [{ id:'questions', label:'ตอบคำถาม / เติมคำ', hint:'เหมาะกับการทบทวนความเข้าใจและตรวจความรู้พื้นฐาน' }, { id:'table', label:'ตาราง / จำแนกข้อมูล', hint:'เหมาะกับการจัดกลุ่ม เปรียบเทียบ จับคู่ หรือเติมข้อมูล' }, { id:'inquiry', label:'ทดลอง / สำรวจ', hint:'มีขั้นตอน ตารางบันทึกผล คำถามวิเคราะห์ และสรุปผล' }, { id:'scenario', label:'วิเคราะห์สถานการณ์', hint:'ให้นักเรียนใช้ข้อมูลหรือหลักฐานเพื่ออธิบายและเสนอแนวทาง' }, { id:'product', label:'สร้างชิ้นงาน', hint:'มีโจทย์ ผลงานที่ต้องส่ง พื้นที่ออกแบบ และเกณฑ์ประเมิน' }];
  ...
  function worksheetFormHtml() {
    const saved = state.worksheetDraft || {}; const prefill = state.worksheetPrefill || {}; const selectedType = saved.worksheetType || prefill.worksheetType || state.worksheetType; state.worksheetType = selectedType;
    const formatBlock = `<section class="cl-worksheet-format-card"><div><h3>2. เลือกรูปแบบใบงาน</h3><p>ใบงานแต่ละชนิดมีโครงสร้างไม่เหมือนกัน ระบบจะใช้รูปแบบนี้เป็นกรอบในการสร้างร่าง</p></div><label class="cl-field"><span>รูปแบบใบงาน <em class="cl-required">*</em></span><select id="worksheet-type" class="form-control" onchange="setWorksheetType(this.value)">${worksheetFormats.map(item => `<option value="${item.id}" ${item.id === selectedType ? 'selected' : ''}>${item.label}</option>`).join('')}</select><small class="cl-field-hint">${escapeHtml(worksheetFormat(selectedType).hint)}</small></label></section>`;
    return legacyWorksheetFormHtml().replace('<h3>2. สิ่งที่ต้องการให้ใบงานช่วยจัดทำ</h3>', `${formatBlock}<h3>3. รายละเอียดที่ต้องการให้ใบงานช่วยจัดทำ</h3>`);
  }
  ...
  window.generateWorksheetDraft = async function () {
    ...
    const payload = { title, worksheetType, worksheetTypeLabel:worksheetFormat(worksheetType).label, classId:room.id, classLabel:`${room.subject} · ${room.className}`, subject:room.subject, grade:gradeLabel(state.grade), topic:state.worksheetPrefill?.topic || title, activityType:document.getElementById('worksheet-activity-type')?.value || '', duration:document.getElementById('worksheet-duration')?.value || '', indicators:indicators.map(row => ({ code:row.code, text:row.text })), learnerOutput:document.getElementById('worksheet-output')?.value.trim() || '', resources:document.getElementById('worksheet-resources')?.value.trim() || '', focus:document.getElementById('worksheet-focus')?.value.trim() || '', lessonPlanId:state.worksheetPrefill?.lessonPlanId || '' };
  ...
  window.openLessonPlanCreator = function () { state.worksheetView = 'home'; state.lessonView = 'form'; state.lessonDraft = null; state.lessonDraftOrigin = 'new'; state.lessonEditing = false; state.lessonQuizLinkOpen = false; state.lessonWorksheetLinkOpen = false; state.lessonIndicatorCodes = []; render(); };
  window.closeLessonPlanCreator = function () { state.worksheetView = 'home'; state.lessonView = 'home'; state.lessonDraft = null; state.lessonDraftOrigin = 'new'; state.lessonEditing = false; state.lessonQuizLinkOpen = false; state.lessonWorksheetLinkOpen = false; render(); };
  ...
  function lessonFormHtml() {
    ...
    return `<div class="cl-wizard cl-lesson-wizard"><div class="cl-wizard-top"><button class="btn" type="button" onclick="closeLessonPlanCreator()"><i class="hgi-stroke hgi-arrow-left-01"></i> <span>กลับคลัง</span></button><div class="cl-wizard-title"><span class="cl-create-icon lesson-plan"><i class="hgi-stroke hgi-presentation-01"></i></span><div><h2>สร้างแผนการสอน</h2><p>กำหนดกรอบสำคัญ แล้วให้ AI ช่วยเติมร่างแผน</p></div></div></div>
      ...
      <section class="card cl-form-card"><h3>1. ข้อมูลพื้นฐาน</h3><p class="cl-form-note">เลือกข้อมูลที่มีอยู่ใน ClassKru ก่อน เพื่อให้แผนสอดคล้องกับหลักสูตรและห้องเรียนจริง</p><div class="cl-form-grid">
  ...
  window.generateLessonPlanDraft = async function () {
    const room = selectedClass(); const topic = document.getElementById('lesson-topic')?.value.trim() || ''; const indicators = selectedLessonIndicators();
    ...
    if (!indicators.length) { showToast('กรุณาเลือกตัวชี้วัดอย่างน้อย 1 ข้อ', 'warning'); return; }
    const payload = { title:topic, classId:room.id, classLabel:`${room.subject} · ${room.className}`, subject:room.subject, grade:gradeLabel(state.grade), topic, duration:document.getElementById('lesson-duration')?.value || '', method:document.getElementById('lesson-method')?.value || '', indicators:indicators.map(row => ({ code:row.code, text:row.text })), learnerContext:document.getElementById('lesson-learners')?.value.trim() || '', resources:document.getElementById('lesson-resources')?.value.trim() || '', focus:document.getElementById('lesson-focus')?.value.trim() || '' };
