...
(function () {
  ...
  function savedWorksheetHtml(item) { return `<button class="cl-saved-card worksheet" type="button" onclick="openSavedWorksheet('${escapeHtml(item.id)}')"><span class="cl-saved-type cl-worksheet-type"><i class="hgi-stroke hgi-note-02"></i> ใบงาน</span><h4>${escapeHtml(item.title || 'ใบงานไม่มีชื่อ')}</h4><p>${escapeHtml(item.classLabel || 'ยังไม่ได้เชื่อมห้องเรียน')}</p><div class="cl-saved-meta"><span>${escapeHtml(item.activityType || 'กิจกรรม')}</span><span>${Array.isArray(item.indicators) ? item.indicators.length : 0} ตัวชี้วัด</span><span>ร่าง</span></div><span class="cl-saved-open">เปิดใบงาน <i class="hgi-stroke hgi-arrow-right-01"></i></span></button>`; }
  ...
  window.openWorksheetCreator = function () { state.worksheetView = 'form'; state.worksheetDraft = null; state.worksheetDraftOrigin = 'new'; state.worksheetEditing = false; state.worksheetPrefill = null; state.worksheetIndicatorCodes = []; render(); };
  ...
  window.generateWorksheetDraft = async function () { const room = selectedClass(); const title = document.getElementById('worksheet-title')?.value.trim() || ''; const indicators = selectedWorksheetIndicators(); if (!room) { showToast('กรุณาเลือกห้องเรียนก่อนสร้างใบงาน', 'warning'); return; } if (title.length < 3) { showToast('กรุณาระบุชื่อใบงาน', 'warning'); return; } if (!indicators.length) { showToast('กรุณาเลือกตัวชี้วัดอย่างน้อย 1 ข้อ', 'warning'); return; } const payload = { title, classId:room.id, classLabel:`${room.subject} · ${room.className}`, subject:room.subject, grade:gradeLabel(state.grade), topic:state.worksheetPrefill?.topic || title, activityType:document.getElementById('worksheet-activity-type')?.value || '', duration:document.getElementById('worksheet-duration')?.value || '', indicators:indicators.map(row => ({ code:row.code, text:row.text })), learnerOutput:document.getElementById('worksheet-output')?.value.trim() || '', resources:document.getElementById('worksheet-resources')?.value.trim() || '', focus:document.getElementById('worksheet-focus')?.value.trim() || '', lessonPlanId:state.worksheetPrefill?.lessonPlanId || '' }; ensureWorksheetProcessingOverlay(); document.getElementById('worksheet-processing-overlay')?.classList.add('show'); try { const session = await supabaseClient?.auth?.getSession(); const token = session?.data?.session?.access_token; const response = await fetch(apiPath('/api/ai/generate-worksheet'), { method:'POST', headers:{ 'Content-Type':'application/json', ...(token ? { Authorization:`Bearer ${token}` } : {}) }, body:JSON.stringify(payload) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.message || result.error || 'ไม่สามารถสร้างใบงานได้'); state.worksheetDraft = { ...payload, id:uid(), createdAt:new Date().toISOString(), worksheet:result.worksheet, warnings:Array.isArray(result.warnings) ? result.warnings : [] }; state.worksheetDraftOrigin = 'new'; state.worksheetEditing = false; state.worksheetView = 'review'; render(); } catch (error) { console.warn('Worksheet generation failed:', error); showToast(`สร้างใบงานไม่สำเร็จ: ${error.message || 'กรุณาลองใหม่'}`, 'warning', 7000); } finally { document.getElementById('worksheet-processing-overlay')?.classList.remove('show'); } };
