...
(function () {
  ...
  function worksheetOptionsHtml(saved, prefill) {
    ...
    return `<section class="cl-worksheet-options-card"><div class="cl-worksheet-options-head"><div><h3>3. กำหนดลักษณะใบงาน</h3><p>กำหนดจำนวนข้อ/แถวที่นักเรียนต้องทำรวมทั้งหมด ระบบจะจัดโจทย์ลงในใบงานให้</p></div><span class="cl-worksheet-guide-badge"><i class="hgi-stroke hgi-magic-wand-01"></i> จัดหน้าพร้อมพื้นที่ตอบ</span></div><div class="cl-form-grid">${option('work-mode', value('worksheetWorkMode', 'individual'), { label:'รูปแบบการทำงาน', options:[{value:'individual',label:'ทำรายบุคคล'},{value:'pair',label:'ทำงานเป็นคู่'},{value:'group',label:'ทำงานเป็นกลุ่ม'}] })}${option('item-count', value('worksheetItemCount', '8'), { label:'จำนวนข้อ/แถวที่ต้องทำ', options:[{value:'4',label:'4 ข้อ/แถว'},{value:'6',label:'6 ข้อ/แถว'},{value:'8',label:'8 ข้อ/แถว'},{value:'10',label:'10 ข้อ/แถว'}] })}${option('difficulty', value('worksheetDifficulty', 'medium'), { label:'ระดับความยาก', options:[{value:'easy',label:'พื้นฐาน'},{value:'medium',label:'ปานกลาง'},{value:'hard',label:'ท้าทาย'}] })}${option('visuals', value('worksheetVisuals', 'none'), { label:'สื่อประกอบที่รองรับ', options:[{value:'none',label:'ข้อความและตาราง (ยังไม่รองรับภาพ)'}] })}${option('answer-space', value('worksheetAnswerSpace', 'medium'), { label:'พื้นที่คำตอบ', options:[{value:'short',label:'สั้น กระชับ'},{value:'medium',label:'พอดี'},{value:'long',label:'มีพื้นที่อธิบาย'}] })}${option('answer-key', value('worksheetAnswerKey', 'yes'), { label:'เฉลยสำหรับครู', options:[{value:'yes',label:'สร้างเฉลยด้วย'},{value:'no',label:'ไม่ต้องสร้างเฉลย'}] })}</div></section>`;
  }
