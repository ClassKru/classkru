...
(function () {
  ...
  window.openLessonPlanCreator = function () { state.lessonView = 'form'; state.lessonDraft = null; state.lessonIndicatorCodes = []; render(); };
  window.closeLessonPlanCreator = function () { state.lessonView = 'home'; state.lessonDraft = null; render(); };
  ...
  function lessonFormHtml() {
    ...
    return `<div class="cl-wizard cl-lesson-wizard"><div class="cl-wizard-top"><button class="btn" type="button" onclick="closeLessonPlanCreator()"><i class="hgi-stroke hgi-arrow-left-01"></i> <span>กลับคลัง</span></button><div class="cl-wizard-title"><span class="cl-create-icon lesson-plan"><i class="hgi-stroke hgi-presentation-01"></i></span><div><h2>สร้างแผนการสอน</h2><p>กำหนดกรอบสำคัญ แล้วให้ AI ช่วยเติมร่างแผน</p></div></div></div>
      ...
      <section class="card cl-form-card"><h3>1. ข้อมูลพื้นฐาน</h3><p class="cl-form-note">เลือกข้อมูลที่มีอยู่ใน ClassKru ก่อน เพื่อให้แผนสอดคล้องกับหลักสูตรและห้องเรียนจริง</p><div class="cl-form-grid">
        ...
        <label class="cl-field full"><span>หัวข้อบทเรียน <em class="cl-required">*</em></span><input id="lesson-topic" class="form-control" maxlength="240" placeholder="เช่น การถ่ายทอดลักษณะทางพันธุกรรม"></label>
        <label class="cl-field"><span>เวลาเรียน</span><select id="lesson-duration" class="form-control"><option>1 คาบ (50 นาที)</option><option>2 คาบ (100 นาที)</option><option>3 คาบ (150 นาที)</option><option>ปรับเอง</option></select></label>
        <label class="cl-field"><span>รูปแบบการเรียนรู้</span><select id="lesson-method" class="form-control"><option>สืบเสาะหาความรู้</option><option>ลงมือปฏิบัติ</option><option>ร่วมมือกันเรียนรู้</option><option>ใช้ปัญหาเป็นฐาน</option><option>ผสมผสาน</option></select></label>
      </div><div class="cl-form-grid"><div class="cl-field full"><span>ตัวชี้วัด/ผลการเรียนรู้ <small>(เลือกได้หลายข้อ)</small></span><div class="cl-indicator-table" role="group" aria-label="เลือกตัวชี้วัดสำหรับแผนการสอน"><div class="cl-indicator-table-head"><span></span><span>รหัส</span><span>รายละเอียดตัวชี้วัด</span></div>${indicatorRows.slice(0,18).map(i => `<label class="cl-indicator-row"><input type="checkbox" value="${escapeHtml(i.code)}" ${state.lessonIndicatorCodes.includes(i.code) ? 'checked' : ''} onchange="toggleLessonIndicator(this.value,this.checked)"><strong>${escapeHtml(i.code)}</strong><span>${escapeHtml(i.text)}</span></label>`).join('') || '<div class="cl-indicator-empty">เลือกกลุ่มสาระและระดับชั้นเพื่อแสดงตัวชี้วัด</div>'}</div></div></div>
      <h3>2. บริบทที่ต้องการให้ AI นำไปใช้</h3><p class="cl-form-note">กรอกเท่าที่รู้ ระบบจะไม่สร้างข้อมูลผลการเรียนแทนข้อมูลจริง</p><div class="cl-form-grid"><label class="cl-field full"><span>ลักษณะผู้เรียน/ปัญหาที่พบ</span><textarea id="lesson-learners" class="form-control" rows="3" maxlength="1600" placeholder="เช่น นักเรียนยังสับสนเรื่อง... หรือมีพื้นฐานแตกต่างกัน"></textarea></label><label class="cl-field full"><span>อุปกรณ์หรือข้อจำกัดของห้องเรียน</span><textarea id="lesson-resources" class="form-control" rows="2" maxlength="1000" placeholder="เช่น มีอุปกรณ์ทดลอง 5 ชุด ใช้โทรศัพท์ได้บางส่วน"></textarea></label><label class="cl-field full"><span>สิ่งที่ครูอยากเน้นเป็นพิเศษ</span><textarea id="lesson-focus" class="form-control" rows="2" maxlength="1000" placeholder="เช่น เน้นการอธิบายด้วยหลักฐาน และให้มีการประเมินระหว่างเรียน"></textarea></label></div>
  ...
  window.generateLessonPlanDraft = async function () {
    ...
    try { const session = await supabaseClient?.auth?.getSession(); const token = session?.data?.session?.access_token; const response = await fetch('/api/ai/generate-lesson-plan', { method:'POST', headers:{ 'Content-Type':'application/json', ...(token ? { Authorization:`Bearer ${token}` } : {}) }, body:JSON.stringify(payload) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.message || result.error || 'ไม่สามารถสร้างแผนการสอนได้'); state.lessonDraft = { ...payload, id:uid(), createdAt:new Date().toISOString(), plan:result.plan, warnings:Array.isArray(result.warnings) ? result.warnings : [] }; state.lessonView = 'review'; render(); } catch (error) { console.warn('Lesson plan generation failed:', error); showToast(`สร้างแผนไม่สำเร็จ: ${error.message || 'กรุณาลองใหม่'}`, 'warning', 7000); } finally { lessonProcessing(false); }
  };
  ...
  function lessonReviewHtml() { const d = state.lessonDraft; if (!d) return lessonFormHtml(); const p = d.plan || {}; return `<div class="cl-wizard cl-lesson-wizard"><div class="cl-wizard-top"><button class="btn" type="button" onclick="backToLessonSettings()"><i class="hgi-stroke hgi-arrow-left-01"></i> <span>แก้กรอบแผน</span></button><div class="cl-wizard-title"><span class="cl-create-icon lesson-plan"><i class="hgi-stroke hgi-presentation-01"></i></span><div><h2>ตรวจร่างแผนการสอน</h2><p>${escapeHtml(d.classLabel)} · ${escapeHtml(d.topic)}</p></div></div></div><div class="cl-stepper"><span class="cl-step"><b>1</b> กำหนดกรอบ</span><span class="cl-step active"><b>2</b> ตรวจร่าง</span><span class="cl-step"><b>3</b> บันทึกเข้าคลัง</span></div>${d.warnings?.length ? `<div class="cl-quality-warning"><i class="hgi-stroke hgi-alert-02"></i><div><strong>ข้อควรตรวจสอบ</strong><p>${d.warnings.map(escapeHtml).join('<br>')}</p></div></div>` : ''}<section class="card cl-plan-preview"><header><span class="cl-saved-type cl-plan-type"><i class="hgi-stroke hgi-presentation-01"></i> ร่างโดย AI</span><h2>${escapeHtml(p.title || d.topic)}</h2><p>${escapeHtml(d.classLabel)} · ${escapeHtml(d.duration)} · ${escapeHtml(d.method)}</p></header><div class="cl-plan-grid"><article><h3>สาระสำคัญ</h3>${listHtml(p.keyConcepts)}</article><article><h3>จุดประสงค์การเรียนรู้</h3>${listHtml(p.objectives)}</article><article><h3>ขั้นตอนกิจกรรม</h3>${listHtml(p.activities)}</article><article><h3>สื่อและแหล่งเรียนรู้</h3>${listHtml(p.resources)}</article><article><h3>การวัดและประเมินผล</h3>${listHtml(p.assessment)}</article><article><h3>หลักฐานที่ควรเก็บ</h3>${listHtml(p.evidence)}</article><article class="cl-plan-wide"><h3>คำแนะนำการปรับใช้</h3>${listHtml(p.adaptations)}</article></div></section><div class="cl-review-bottom"><span>ร่างนี้สร้างจากตัวชี้วัดและบริบทที่ครูกำหนด ครูควรตรวจสอบก่อนนำไปใช้</span><div class="cl-actions"><button class="btn" type="button" onclick="backToLessonSettings()">สร้างใหม่</button><button class="btn btn-primary" type="button" onclick="saveLessonPlanDraft()"><i class="hgi-stroke hgi-floppy-disk"></i> บันทึกเข้าคลัง</button></div></div></div>`; }
  window.backToLessonSettings = function () { state.lessonView = 'form'; render(); };
  window.saveLessonPlanDraft = function () { if (!state.lessonDraft?.plan) { showToast('ยังไม่มีร่างแผนการสอน', 'warning'); return; } appState.mediaLibrary = library().filter(item => item.id !== state.lessonDraft.id); appState.mediaLibrary.push({ ...state.lessonDraft, status:'draft', type:'lesson_plan' }); saveState(); state.lessonView = 'home'; state.lessonDraft = null; render(); showToast('บันทึกแผนการสอนเข้าคลังแล้ว'); };
  window.openSavedLessonPlan = function (id) { const plan = library().find(item => item.id === id && item.type === 'lesson_plan'); if (!plan) { showToast('ไม่พบแผนการสอนนี้', 'warning'); return; } state.lessonDraft = plan; state.lessonView = 'review'; render(); };
