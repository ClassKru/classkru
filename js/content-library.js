...
(function () {
  ...
  function worksheetOptionsHtml(saved, prefill) {
    ...
    return `<section class="cl-worksheet-options-card"><div class="cl-worksheet-options-head"><div><h3>3. กำหนดกรอบให้ AI</h3><p>เลือกเท่าที่ต้องการได้เลย ClassKru จะสร้างคำสั่งให้ API อัตโนมัติ</p></div><span class="cl-worksheet-guide-badge"><i class="hgi-stroke hgi-magic-wand-01"></i> ไม่ต้องเขียน Prompt</span></div><div class="cl-form-grid">${option('work-mode', value('worksheetWorkMode', 'individual'), { label:'รูปแบบการทำงาน', options:[{value:'individual',label:'ทำรายบุคคล'},{value:'pair',label:'ทำงานเป็นคู่'},{value:'group',label:'ทำงานเป็นกลุ่ม'}] })}${option('item-count', value('worksheetItemCount', '8'), { label:'จำนวนกิจกรรม/ข้อ', options:[{value:'4',label:'4 รายการ'},{value:'6',label:'6 รายการ'},{value:'8',label:'8 รายการ'},{value:'10',label:'10 รายการ'}] })}${option('difficulty', value('worksheetDifficulty', 'medium'), { label:'ระดับความยาก', options:[{value:'easy',label:'พื้นฐาน'},{value:'medium',label:'ปานกลาง'},{value:'hard',label:'ท้าทาย'}] })}${option('visuals', value('worksheetVisuals', 'none'), { label:'ภาพหรือสื่อประกอบ', options:[{value:'none',label:'ไม่เน้นภาพ'},{value:'simple',label:'มีภาพ/แผนภาพอย่างง่าย'},{value:'provided',label:'ใช้ภาพที่ครูกำหนด'}] })}${option('answer-space', value('worksheetAnswerSpace', 'medium'), { label:'พื้นที่คำตอบ', options:[{value:'short',label:'สั้น กระชับ'},{value:'medium',label:'พอดี'},{value:'long',label:'มีพื้นที่อธิบาย'}] })}${option('answer-key', value('worksheetAnswerKey', 'yes'), { label:'เฉลยสำหรับครู', options:[{value:'yes',label:'สร้างเฉลยด้วย'},{value:'no',label:'ไม่ต้องสร้างเฉลย'}] })}</div></section>`;
  }
