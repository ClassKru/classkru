...
const TEACHING_TOOLS = [
  { id:'media-studio',name:'สร้างสื่อกับ AI',desc:'สร้างเกมและเปิดคลังสื่อของฉัน',icon:'hgi-magic-wand-01',tint:'purple',status:'ready',open:()=>openInteractiveMediaStudio() },
...
function openCountdownTool() {
  ...
  _tlOpen('tool-countdown-overlay', `
      ...
      <div class="tl-custom-row">
        <label>กำหนดเอง <input type="number" id="cd-custom-min" min="1" max="999" value="5" inputmode="numeric"> นาที</label>
        <button class="tl-mini-btn" onclick="_cdCustom()">ใช้เวลา</button>
      </div>
      <div class="tl-btns">
...
function _cdSetTotal(sec, presetMin) {
  _cdStop(); _cdEnd = 0;
  _cdTotal = Math.max(1, Math.round(sec));
  _cdLeft = _cdTotal;
  document.querySelectorAll('#cd-chips .tl-chip').forEach(b => b.classList.toggle('on', Number(b.dataset.min) === presetMin));
  const input = document.getElementById('cd-custom-min');
  if (input) input.value = Math.max(1, Math.round(_cdTotal / 60));
  const b = document.getElementById('cd-toggle'); if (b) b.textContent = 'เริ่ม';
  _cdPaint();
}
function _cdPreset(min) {
  _cdSetTotal(min * 60, min);
}
function _cdCustom() {
  const input = document.getElementById('cd-custom-min');
  const min = Math.max(1, Math.min(999, Math.round(Number(input && input.value) || 5)));
  _cdSetTotal(min * 60, CD_PRESETS.includes(min) ? min : null);
}
function _cdPaint() {
...
function openRandomGroupTool() {
  ...
  _tlOpen('tool-group-overlay', `
      ...
      <div class="tl-custom-row">
        <label>จำนวนกลุ่ม <input type="number" id="gr-custom-count" min="1" max="99" value="4" inputmode="numeric"> กลุ่ม</label>
        <button class="tl-mini-btn" onclick="_grCustomCount()">ใช้จำนวนนี้</button>
      </div>
...
function _grSetCount(n) {
  _grCount = Math.max(1, Math.min(99, Math.round(Number(n) || 4)));
  document.querySelectorAll('#gr-chips .tl-chip').forEach(b => b.classList.toggle('on', Number(b.dataset.n) === _grCount));
  const input = document.getElementById('gr-custom-count');
  if (input) input.value = _grCount;
  _tlSync();
}
function _grCustomCount() {
  const input = document.getElementById('gr-custom-count');
  const n = Math.max(1, Math.min(99, Math.round(Number(input && input.value) || 4)));
  _grSetCount(n);
}
let _grBusy = false;
