...
>
> เวอร์ชัน 446: แก้ห้องหายหลังรีเฟรชเมื่อเคยล้างข้อมูล: เลิกใช้ `classkru_skip_sync` เพื่อข้ามการเริ่ม relational writer (เดิม cached ready แต่ teacher ID ว่าง ทำให้ save คืน false โดยไม่มีคิว). หลัง reset ให้เชื่อมต่อและโหลดบัญชีตามปกติ; ปิดกรอบไกด์ของฟอร์มที่ปิดไปแล้วหากบันทึกล้มเหลว. `tests/reset-refresh.test.cjs` จำลอง storage/server ข้าม reload, ยืนยันการบันทึกห้องแรกและการล้างเฉพาะ cache; โค้ด v445 ไม่ผ่านการทดสอบนี้ แต่ v446 และ regression ทั้งหมดผ่าน. ยังไม่ได้ยืนยันการกู้ห้องที่หายไปก่อนแก้จากบัญชีผู้ใช้จริง.
> เวอร์ชัน 447: แก้ทัวร์ค้างเมื่อ Cloud ตอบช้าหรือบันทึกไม่สำเร็จ: ไกด์เดินต่อจาก local-first save ได้ และแสดงคำเตือนให้คิว relational retry เบื้องหลัง; เพิ่ม regression ให้ยืนยันทั้งกรณีเขียนสำเร็จและล้มเหลว.
> เวอร์ชัน 448: แก้ปุ่มล้างข้อมูลไม่เปิด welcome guide และทัวร์ค้างเมื่อมีห้องจาก Cloud: เพิ่ม `classkru_onboarding_preview`, ให้ resume onboarding ที่ยังไม่เสร็จ และข้ามขั้นรอสร้างห้องเมื่อมีห้องอยู่แล้ว. Regression และ JS syntax ผ่าน.
> เวอร์ชัน 449: แยก local working copy ตามบัญชีด้วย key ที่ผูกกับอีเมล แก้ ID ใหม่เห็นห้องจากบัญชีก่อนและไม่เข้า welcome guide; `onLoginSuccess` ตั้ง key ก่อนโหลด state. ทดสอบ account isolation, reset/refresh และ regression ทั้งหมดผ่าน.
> เวอร์ชัน 450: แก้การสลับบัญชีในแท็บเดิม: เมื่อบัญชีใหม่ไม่มี local state ให้แทนที่ `appState` ทั้งก้อน เพื่อไม่ให้ `onboarding.done` ของบัญชีก่อนติดค้างจนไม่แสดง welcome guide. Regression และ JS syntax ผ่าน.
> เวอร์ชัน 451: แก้ onboarding ไม่เปิดเมื่อสลับบัญชีในแท็บเดิม: reset `onboardingChecked` และปิดทัวร์/โมดัลค้างทุกครั้งที่ login สำเร็จ ก่อนเริ่ม sync และตรวจ onboarding ของบัญชีใหม่. Regression และ JS syntax ผ่าน.
> เวอร์ชัน 452: ปรับ first-use onboarding ให้บัญชีที่ยังไม่มี `onboarding.done` เห็น Welcome Guide เสมอหลัง login แม้มีห้องถูกกู้จาก Cloud แล้ว; ไม่ผูกการแสดงไกด์กับจำนวนห้อง. Regression และ JS syntax ผ่าน.
>
